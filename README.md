...
```
Generates vulnerability layers for 4 temporal scenarios using POTI data and time windows. The scenarios and the time-window table are defined in `data/time_windows/scenarios_T1g.csv` and `data/time_windows/time_windows_T1g.csv` (see `docs/thesis/README.md`); requires `verus>=1.1.2`, which clusters only the POTIs active at each evaluation time. The article's original setup and results are in git commit `7cef902`.
